  ...
  return container.exec(options, (err, exec) => {
    ...
    exec.start(attachoptions, (err, execStream) => {
      ...
      var strout = through((data) => {
        ...
        socket.send(String.fromCharCode(1) + data.toString(), { binary: true, mask: true }); //maybe switch to buffer or base64?
      });
      var strerr = through((data) => {
        if (dbgstream) {
          dbgstream.write('ERR: ' + data + '\n-\n');
        }
        socket.send(String.fromCharCode(2) + data.toString(), { binary: true, mask: true });
      });
