...
class ExecSession {
  ...
  async execute () {
    ...
    let execStream = await exec.start(this.attachOptions);
    ...
    exec.modem.demuxStream(this.execStream, this.strout, this.strerr);
    this.strbuf = through();
  ...
  messageHandler (message) {
    ...
    var buf = message; //really necessary when we're not using arraybuffer?
    ...
    switch (message[0]) {
      ...
      case msgcode.stdin:
        if (!this.execStream.write(buf.slice(1), {binary: true})) {
...
export default class DockerExecWebsocketServer {
  ...
  close () {
    this.server.close();
    this.sessions.foreach((session) => { //could be improved later
      session.close();
    });
  }
}
