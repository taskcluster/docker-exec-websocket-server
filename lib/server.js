...
function execWithSocket (container, socket, command, tty, dbgWrite) {
  ...
  container.exec(options).then((_exec) => {
    ...
    var strout = through((data) => {
      dbgWrite('OUT', data);
      socket.send(muxBuffer(msgcode.stdout, data), {binary: true, mask: true});
    });
    var strerr = through((data) => {
      dbgWrite('ERR', data);
      socket.send(muxBuffer(msgcode.stderr, data), {binary: true, mask: true});
    });
    ...
    execStream.on('end', () => {
      exec.inspect((err, data) => {
        ...
        var exitBuf = new Buffer(4);
        debug(data.ExitCode);
        exitBuf.writeInt32LE(data.ExitCode);
        socket.send(muxBuffer(msgcode.stopped, exitBuf));
      });
    });

    //send ready message to let client know it is ready
    socket.send(new Buffer([msgcode.resume]), {binary: true, mask: true}); 
  }).done();
}
...
export default class DockerExecWebsocketServer {
   ...
   */
  constructor (options) {
    options = _.defaults({}, options, {path: '/'+slugid.v4(), tty: false, log: false});
