...
function execWithSocket (container, socket, command, tty, dbgWrite) {
  ...
  container.exec(options).then((_exec) => {
    ...
    var strout = through(function (data) {
      dbgWrite('out', data);
      var buf = muxBuffer(msgcode.stdout, new Buffer(data));
      this.queue(buf);
    });

    var strerr = through(function (data) {
      dbgWrite('err', data);
      var buf = muxBuffer(msgcode.stderr, new Buffer(data));
      this.queue(buf);
    });

    exec.modem.demuxStream(execStream, strout, strerr);
    var strbuf = through();
    strbuf.pipe(through((data) => {
      ...
      switch (message[0]) {
        ...
        case msgcode.stdin:
          if (!execStream.write(buf.slice(1), {binary: true})) {
            strbuf.write(new Buffer([msgcode.pause]), {binary: true});
            debug('paused');
          }
          // debug('server needs drain: %s',!tf);
          break;
    ...
    execStream.on('drain', () => {
      strbuf.write(new Buffer([msgcode.resume]), {binary: true});
      debug('resumed');
    });
    ...
    execStream.on('end', () => {
      exec.inspect().then((data) => {
        ...
        strbuf.write(muxBuffer(msgcode.stopped, exitBuf));
      }).done();
    });

    //send ready message to let client know it is ready
    strbuf.write(new Buffer([msgcode.resume]));
  }).done();
}
